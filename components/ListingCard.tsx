...
const ListingCard = forwardRef<HTMLDivElement, ListingCardProps>(
  ({ listing, onStartChat, suitabilityScore, suitabilityReason, isSelected, onClick, isInWishlist, onToggleWishlist }, ref) => {

    return (
      <div 
        ref={ref}
        onClick={onClick}
        className={`bg-surface dark:bg-gray-800 rounded-lg shadow-md overflow-hidden hover:shadow-xl dark:hover:shadow-primary/20 transition-all duration-300 ${isSelected ? 'ring-2 ring-primary ring-offset-2 dark:ring-offset-gray-900' : ''} ${onClick ? 'cursor-pointer' : ''}`}
      >
        <div className="relative">
          ...
          >
            <span className={`material-icons-outlined ${isInWishlist ? 'text-red-500' : ''}`}>
          ...
          <div className="flex justify-between items-center mt-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button 
                onClick={(e) => {
                   e.stopPropagation(); // Prevent card's onClick from firing
                   onStartChat({ userName: listing.seller, userPhoto: listing.sellerPhoto, listingTitle: listing.title })
                }}
